...
import type { CustomerProfile, TagPool, OneIdOption, SourceDetail, MobileItem, MobileData, MaintenanceRecord, TransactionRecord, VehicleRelation, Asset, ConflictResolution, Appointment, PlatformSource, Opportunity, OperationLog, InsuranceRecord, MarketingCampaign, FinancialLoanRecord } from '@/types/customer'
...
// 导出类型供外部使用
export type { CustomerProfile, TagPool, OneIdOption, SourceDetail, MobileItem, MobileData, MaintenanceRecord, TransactionRecord, VehicleRelation, Asset, ConflictResolution, Appointment, PlatformSource, Opportunity, OperationLog, InsuranceRecord, MarketingCampaign, FinancialLoanRecord }
...
export const customerApi = {
  // 根据手机号获取关联的 OneId 列表（多 oneId 时用于顶部切换）
  getOneIdsByPhone: (phone: string): Promise<ApiResponse<OneIdOption[]>> => {
    return request.get('/customer/oneids-by-phone', {
