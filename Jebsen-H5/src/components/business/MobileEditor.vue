            ...
            >
              <div class="mobile-item-header">
                <div class="mobile-number">
                  ...
                  <!-- 非编辑模式：显示姓名（若有）、号码和标签（车辆在下方列表展示，此处不展示） -->
                  <template v-else>
                    <span v-if="item.contactName" class="contact-name">{{ item.contactName }}</span>
                ...
                <div v-if="editingItemId !== item.id" class="mobile-actions">
                  <!-- 非编辑模式：显示操作图标 -->
                  <van-icon
...
.relation-selector {
  ...
  // 车辆标签区：复用相同 tag 样式，并保证标签之间有统一间距
  .vehicle-tag-section {
    .tag-options {
      display: flex;
      flex-wrap: wrap;
      gap: 6px; // 与上方关系标签、业务标签一致
    }
  }
}
