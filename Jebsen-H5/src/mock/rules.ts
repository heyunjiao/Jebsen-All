...
export interface InsuranceRecordRules {
  ...
  optional: {
    ...
    renewalSpecialistName: (value: any) => boolean // 续保专员名字
  }
  // 业务规则
  business: {
...
export function validateInsuranceRecord(record: any): record is InsuranceRecord {
  ...
  if (record.renewalSpecialistName !== undefined && typeof record.renewalSpecialistName !== 'string') {
    console.error('[Rules] 保险记录验证失败: renewalSpecialistName 必须是字符串', record)
    return false
  }
...
export function normalizeInsuranceRecord(record: Partial<InsuranceRecord>): InsuranceRecord {
  const normalized: InsuranceRecord = {
    ...
    ...(record.commercialInsuranceExpiryDate && { commercialInsuranceExpiryDate: record.commercialInsuranceExpiryDate }),
    ...(record.insuredPerson != null && record.insuredPerson !== '' && { insuredPerson: record.insuredPerson }),
    ...(record.vehicleDamageAmount != null && { vehicleDamageAmount: record.vehicleDamageAmount }),
    ...(record.driverSeatAmount != null && { driverSeatAmount: record.driverSeatAmount }),
    ...(record.passengerSeatAmount != null && { passengerSeatAmount: record.passengerSeatAmount }),
    ...(record.purchaseDate && { purchaseDate: record.purchaseDate }),
    ...(record.source && { source: record.source }),
    ...(record.renewalSpecialistName && { renewalSpecialistName: record.renewalSpecialistName }),
  }
